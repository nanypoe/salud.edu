...
$("#formAgregarPlan").submit(function (e) {
    ...
    $.ajax({
        ...
        type: "POST",
        data: { idEjercicio:idEjercicio, repeticiones: repeticiones, series: series },
        success: function (respuesta) {
            modalFormRespuesta(
                "#modalAgregarPlan",
...
$(".tablaPlan").on("click", ".btnEditarPlan", function () {
    let datos = JSON.parse($(this).attr("data-plan"));
    $("#repeticionesUp").val(datos["repeticiones"]);
    $("#seriesUp").val(datos["series"]);
    $("#idPlan").val(datos["id_plan"]);

    /*ENVIAR al Backend datos de Ejercicios editados*/
    $('#formEditarPlan').submit(function (e) {
        $('#idPlan').prop("disabled", false);
        let idPlan = $("#idPlan").val();
        let repeticiones = $('#repeticionesUp').val();
        let series = $('#seriesUp').val();
        e.preventDefault();
        $.ajax({
            url: 'plan/editarPlan/',
            type: "POST",
            data: {
                idPlan:idPlan, repeticiones:repeticiones, series:series
            },
            success: function (respuesta) {
                modalFormRespuesta(
                    "#modalEditarPlan",
